...
struct TelemetrySnapshot: Sendable, Equatable {
  ...
  var designCapacityMah: UInt16?
  var maxCapacityMah: UInt16?
  ...
  static func create(
    ...
    Self(
      ...
      designCapacityMah: designCapacityMah.map { UInt16(clamping: $0) },
      maxCapacityMah: maxCapacityMah.map { UInt16(clamping: $0) },
...
extension TelemetrySnapshot {
  ...
  @inline(__always)
  var designCapacityInt: Int? { designCapacityMah.map(Int.init) }

  @inline(__always)
  var maxCapacityInt: Int? { maxCapacityMah.map(Int.init) }
  ...
  mutating func setDesignCapacity(_ value: Int?) {
    designCapacityMah = value.map { UInt16(clamping: $0) }
  }

  @inline(__always)
  mutating func setMaxCapacity(_ value: Int?) {
    maxCapacityMah = value.map { UInt16(clamping: $0) }
  }
}

...
final class MainViewModel: ViewModelProtocol {
  ...
  private func handleLaunchAtLoginToggle(_ enabled: Bool) {
    if enabled {
      launchAtLoginService.enableImmediate()
    } else {
      launchAtLoginService.disableImmediate()
    }
  }
  ...
  private func adjustPollingInterval(for snapshot: TelemetrySnapshot) {
    let shouldUseFastPolling =
      snapshot.isBatteryCharging || snapshot.batteryPercent.map { $0 <= 20 } ?? false

...
struct MainView: View {
  ...
  static let gridSpacing: (horizontal: CGFloat, vertical: CGFloat) = (12, 8)
...
private struct TelemetryGridView: View {
  ...
  private var maxCapacity: UInt16? { telemetry.maxCapacityMah }
  private var designCapacity: UInt16? { telemetry.designCapacityMah }
  ...
  var body: some View {
    Grid(
      alignment: .leading, horizontalSpacing: MainView.gridSpacing.horizontal,
      verticalSpacing: MainView.gridSpacing.vertical
    ) {
      BatteryInfoRows(
  ...
  var body: some View {
    ...
    if systemMetrics.network.totalBytesPerSecond > 0 {
      GridRow {
        ...
        TelemetryValue(
          text: systemMetrics.network.formattedBytesPerSecond(
            systemMetrics.network.totalBytesPerSecond) + "/s")
      }
    }
    if systemMetrics.disk.totalBytesPerSecond > 0 {
      GridRow {
        TelemetryLabel.disk
        TelemetryValue(
          text: systemMetrics.disk.formattedBytesPerSecond(systemMetrics.disk.totalBytesPerSecond)
            + "/s")
      }
    }
  }
...
private struct BatteryInfoRows: View {
  ...
  private var chargeLimitSummary: String {
    guard let v = chargeLimitPercent else { return "Requires Battery Daemon" }
    return "\(v)%"
  }
}
...
private struct TelemetryLabel: View {
  ...
  static let cycle = Self(
    icon: "clock.arrow.trianglehead.counterclockwise.rotate.90", text: "Cycle")
  ...
  static let batteryVoltage = Self(
    icon: "minus.plus.and.fluid.batteryblock", text: "Battery Voltage")
  ...
  static let adapterVoltage = Self(
    icon: "minus.plus.and.fluid.batteryblock", text: "Adapter Voltage")

...
struct PowerView: View {
  ...
  static func formatWatts(_ value: Double) -> String {
      ...
      if inputPower > 0.01 {
        if abs(batteryPower) < 0.0001 {
          FlowBar(
            ...
            label: PowerView.formatWatts(inputPower),
            flowWidth: flowWidth,
            flowColor: flowColor,
            middleSectionWidth: middleSectionWidth,
          )
          ...
          VStack(spacing: 0) {
            FlowShaped(
              ...
              direction: 1,
              label: PowerView.formatWatts(batteryPower),
              flowWidth: flowWidth,
              flowColor: flowColor,
              middleSectionWidth: middleSectionWidth,
            )
            FlowShaped(
              ...
              endLength: PowerView.flowHeight,
              direction: 0,
              label: PowerView.formatWatts(systemLoad),
              flowWidth: flowWidth,
              flowColor: flowColor,
              middleSectionWidth: middleSectionWidth,
            )
...
func generateSquircle(width: CGFloat, height: CGFloat, radius: CGFloat, corners: [Bool]) -> Path {
  ...
  let w = (
    width - s0, width - s1, width - s2, width - s3, width - s4, width - s5, width - s6, width - s7
  )
  let h = (
    height - s0, height - s1, height - s2, height - s3, height - s4, height - s5, height - s6,
    height - s7
  )
  ...
  if corners[3] {
    path.addCurve(
      to: CGPoint(x: s1, y: s2), control1: CGPoint(x: s0, y: s4), control2: CGPoint(x: s0, y: s3))
    path.addCurve(
      to: CGPoint(x: s2, y: s1), control1: CGPoint(x: s7, y: s6), control2: CGPoint(x: s6, y: s7))
    path.addCurve(
      to: CGPoint(x: s5, y: s0), control1: CGPoint(x: s3, y: s0), control2: CGPoint(x: s4, y: s0))
  } else {
  ...
  if corners[2] {
    path.addCurve(
      to: CGPoint(x: w.1, y: s1), control1: CGPoint(x: w.3, y: s0), control2: CGPoint(x: w.2, y: s0)
    )
    path.addCurve(
      to: CGPoint(x: w.0, y: s2), control1: CGPoint(x: w.5, y: s7), control2: CGPoint(x: w.6, y: s6)
    )
    path.addCurve(
      to: CGPoint(x: w.0, y: s5), control1: CGPoint(x: w.0, y: s3), control2: CGPoint(x: w.0, y: s4)
    )
  } else {
  ...
  if corners[1] {
    path.addCurve(
      to: CGPoint(x: w.0, y: h.1), control1: CGPoint(x: w.0, y: h.3),
      control2: CGPoint(x: w.0, y: h.2))
    path.addCurve(
      to: CGPoint(x: w.1, y: h.0), control1: CGPoint(x: w.6, y: h.5),
      control2: CGPoint(x: w.5, y: h.6))
    path.addCurve(
      to: CGPoint(x: w.4, y: h.0), control1: CGPoint(x: w.2, y: h.0),
      control2: CGPoint(x: w.3, y: h.0))
  } else {
  ...
  if corners[0] {
    path.addCurve(
      to: CGPoint(x: s2, y: h.0), control1: CGPoint(x: s4, y: h.0), control2: CGPoint(x: s3, y: h.0)
    )
    path.addCurve(
      to: CGPoint(x: s1, y: h.1), control1: CGPoint(x: s6, y: h.6), control2: CGPoint(x: s7, y: h.5)
    )
    path.addCurve(
      to: CGPoint(x: s0, y: h.4), control1: CGPoint(x: s0, y: h.2), control2: CGPoint(x: s0, y: h.3)
    )
  } else {
...
struct FlowClipShape: Shape {
  ...
  func path(in _: CGRect) -> Path {
    flowShape(
      width: width, height: height, startLength: startLength, endLength: endLength,
      direction: direction)
  }
}

func flowShape(
  width: CGFloat, height: CGFloat, startLength: CGFloat, endLength: CGFloat, direction: Int
) -> Path {
  var path = Path()
  ...
  if direction == 0 {
    ...
    path.move(to: .zero)
    path.addCurve(
      to: CGPoint(x: width, y: endY), control1: CGPoint(x: w3, y: 0),
      control2: CGPoint(x: w7, y: endY))
    path.addLine(to: CGPoint(x: width, y: height))
    path.addCurve(
      to: CGPoint(x: 0, y: startLength), control1: CGPoint(x: w7, y: height),
      control2: CGPoint(x: w3, y: startLength))
  } else {
    let startY = height - startLength
    path.move(to: CGPoint(x: 0, y: startY))
    path.addCurve(
      to: CGPoint(x: width, y: 0), control1: CGPoint(x: w3, y: startY),
      control2: CGPoint(x: w7, y: 0))
    path.addLine(to: CGPoint(x: width, y: endLength))
    path.addCurve(
      to: CGPoint(x: 0, y: height), control1: CGPoint(x: w7, y: endLength),
      control2: CGPoint(x: w3, y: height))
  }

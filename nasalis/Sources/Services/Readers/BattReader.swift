...
struct BattReader: Sendable {
  ...
  static func humanReadable(error: Error) -> String {
    if let e = error as? ClientError {
      ...
      case .socketNotFound:
        return "Battery daemon socket not found (daemon not running)"
      case .invalidHTTPResponse:
        return "Invalid HTTP response"
      case .nonOKStatus(let code):
    ...
    if let posix = error as? POSIXError {
      if posix.code.rawValue == 50 {
        return "Battery socket unavailable (daemon not running or permission denied)"
      }
  ...
  private static let queue = DispatchQueue(
    label: "nasalis.battery-daemon-client", qos: .userInitiated, attributes: .concurrent)
  ...
  @inline(__always)
  func fetchUnifiedTelemetry() async throws -> BatteryTelemetryResponse {
    try await requestJSON(path: "/telemetry", as: BatteryTelemetryResponse.self)
  }
    ...
    return try await withCheckedThrowingContinuation { continuation in
      ...
      conn.stateUpdateHandler = { nwState in
        Task {
          switch nwState {
          case .failed(let err):
            if case NWError.posix(let posixErr) = err, posixErr == .ENOENT {
  ...
  private func parseASCIIBool(_ data: Data) -> Bool {
    data.withUnsafeBytes { raw -> Bool in
      ...
      if i + 3 < n {
        let word =
          UInt32(p[i] | 0x20) | (UInt32(p[i + 1] | 0x20) << 8) | (UInt32(p[i + 2] | 0x20) << 16)
...
struct BatteryTelemetryResponse: Decodable, Sendable {
  var power: BatteryPowerTelemetry?
}

struct BatteryPowerTelemetry: Decodable, Sendable {

...
struct SystemMetrics: Sendable, Equatable {
  ...
  var memoryUsage: Double {
    ...
    return min(
      max(memoryUsed.converted(to: .bytes).value / memoryTotal.converted(to: .bytes).value, 0), 1)
  }
  ...
  static let empty = Self(
    ...
    memoryTotal: Measurement(
      value: Double(ProcessInfo.processInfo.physicalMemory) / 1_073_741_824, unit: .gigabytes),
    runningProcesses: 0,
    network: .zero,
    disk: .zero,
...
final class SystemReader: SystemMetricsProviding {
  ...
  private func readMemoryUsage() -> (
    used: Measurement<UnitInformationStorage>, total: Measurement<UnitInformationStorage>
  )? {
  ...
  private func captureNetworkSnapshot() -> NetworkSnapshot? {
    ...
    while let current = pointer?.pointee {
      if let address = current.ifa_addr,
  ...
  private func readDiskThroughput() -> DiskMetrics {
    ...
    let readDelta =
      snapshot.readBytes >= previous.readBytes
      ? snapshot.readBytes - previous.readBytes : snapshot.readBytes
    let writeDelta =
      snapshot.writeBytes >= previous.writeBytes
      ? snapshot.writeBytes - previous.writeBytes : snapshot.writeBytes
  ...
  private func captureDiskSnapshot() -> DiskIOSnapshot? {
    ...
    while case let service = IOIteratorNext(iterator), service != 0 {
      if let property = IORegistryEntryCreateCFProperty(
        service, statsKey as CFString, kCFAllocatorDefault, 0)?
  ...
  private func filterHighActivityProcesses(_ processes: [ProcessUsage], at timestamp: Date)
    -> [ProcessUsage]
  {

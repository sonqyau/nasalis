...
enum ShellReader {
  ...
  static func readBatteryDetails() async -> BatteryDetails {
    ...
    let batteryVoltageV: Double? =
      if let voltageMv {
        Double(voltageMv) / 1000.0
      } else {
        nil
      }

    let batteryAmperageA: Double? =
      if let amperageMa {
        Double(amperageMa) / 1000.0
      } else {
        nil
      }

    let batteryPowerW: Double? =
      if let v = batteryVoltageV, let a = batteryAmperageA {
        v * a
      } else {
        nil
      }
  ...
  private static func runProcess(
    _ executablePath: String, _ arguments: [String], timeoutSeconds: TimeInterval
  ) async -> String? {
